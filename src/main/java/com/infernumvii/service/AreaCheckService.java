...
public class AreaCheckService {
    public boolean isPointInTheArea(Cords cords){
        ...
        boolean exactCircleCond = x.pow(2).add(new BigDecimal(y).pow(2))
        ...
        boolean exactTriangleCond = new BigDecimal(y)
                                        .compareTo(x.add(new BigDecimal(R/2.0f))) <= 0;
        boolean triangleCond = (
                x.compareTo(BigDecimal.ZERO) <= 0 &&
                y >= 0 &&
                y <= R/2.0f && exactTriangleCond
